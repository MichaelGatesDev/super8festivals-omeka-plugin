            ...
            <?php if (count($all_city_records) > 0): ?>
                <?php foreach ($festivals as $festival): ?>
                    <?php if (count($records = get_festival_records_by_type($festival->id, $record_type)) > 0): ?>
                        <div class="row">
                            <div class="col">
                                <h4 class="title"><?= strpos($festival->title, "uncategorized") ? "uncategorized" : $festival->get_title(); ?></h4>
                                <div class="card-columns">
                                    <?php foreach ($records as $record): ?>
                                        <?= $this->partial("__components/cards/video-card.php", array("video" => $record)); ?>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                        </div>
